...
### file%3A%2F%2F%2Fhome%2Fpigeon%2FDownloads%2Fmsc%2FMSc-Project%2Fsequoia%2Freasoner-kernel%2Fsrc%2Fmain%2Fscala%2Fcom%2Fsequoiareasoner%2Fkernel%2Fcontext%2FContextStructureManager.scala:249: error: `)` expected but `val` found
...
//      println("Is empty?: " + provedAtomicQueries(key).isEmpty)
  ...
  val saturationJobs: List[Callable[Unit]] = cs.map(c => {
    val v: Runnable = c.saturateAndPush()
    (() => v.run()): Callable[Unit]
  }).toList
