file://<WORKSPACE>/reasoner-kernel/src/main/scala/com/sequoiareasoner/kernel/context/Context.scala
### scala.reflect.internal.FatalError: 
  unexpected tree: class scala.reflect.internal.Trees$CaseDef
case <ConstantExchange: error>((<originContext: error> @ _), (<coreConstant: error> @ _)) => <state: error> match {
  case (<nomState: error> @ (_: <NominalContextState: error>)) if <nomState: error>.<getCoreConstant: error>.<id: error>.<$eq$eq: error>(coreConstant.id) => ()
  case _ => if (<state: error>.<addConstantToMentionedConstantSet: error>(coreConstant))
    {
      <contextStructureManager: error>.<contextRoundStarted: error>();
      <originContext: error>.<$bang: error>(ConstantMentionedPush(incoming))
    }
  else
    ()
}
     while compiling: file://<WORKSPACE>/reasoner-kernel/src/main/scala/com/sequoiareasoner/kernel/context/Context.scala
  ...
  last tree to typer: CaseDef
       tree position: line 420 of file://<WORKSPACE>/reasoner-kernel/src/main/scala/com/sequoiareasoner/kernel/context/Context.scala
              symbol: null
...
== Source file context for tree position ==
   ...
   418         }
   419 
   420         case ConstantExchange(originContext,coreConstant) => {
   421           /** If the constant corresponding to the core of the origin constant has not been mentioned yet,
   422             * we update the set of mentioned constants, and send a request to `originContext` to send all
   423             * relevant CGCs */
...
action parameters:
offset: 27426
uri: file://<WORKSPACE>/reasoner-kernel/src/main/scala/com/sequoiareasoner/kernel/context/Context.scala
text:
```scala
/*
...
scala.reflect.internal.Reporting.abort(Reporting.scala:70)
	...
	scala.meta.internal.pc.ScalaPresentationCompiler.$anonfun$definition$1(ScalaPresentationCompiler.scala:353)
```
#### Short summary: 
