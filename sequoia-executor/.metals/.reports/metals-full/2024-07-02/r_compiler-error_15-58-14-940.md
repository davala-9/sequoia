...
### file%3A%2F%2F%2Fhome%2Fpigeon%2FDownloads%2Fmsc%2FMSc-Project%2Fsequoia%2Freasoner-kernel%2Fsrc%2Fmain%2Fscala%2Fcom%2Fsequoiareasoner%2Fkernel%2Fcontext%2FContextThreadPool.scala:52: error: `identifier` expected but `while` found
        while (count > 0) {
        ^
...
class ActorExecutionService extends ForkJoinPool {
    ...
    def waitForFinish(): Future[Boolean] = {
        val count: Int = Await.result(counterActor ? 0, Duration(10, MINUTES)).value.
        while (count > 0) {
