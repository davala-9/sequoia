...
### file%3A%2F%2F%2Fhome%2Fpigeon%2FDownloads%2Fmsc%2FMSc-Project%2Fsequoia%2Freasoner-kernel%2Fsrc%2Fmain%2Fscala%2Fcom%2Fsequoiareasoner%2Fkernel%2Fcontext%2FContextThreadPool.scala:28: error: `;` expected but `,` found
        counterActor ! 1, actorRef)
                        ^
...
class ActorExecutionService extends ForkJoinPool {
    ...
    def executeWithPartition(command: Runnable, partitionKey: String): Unit = {
        ...
        counterActor ! 1, actorRef)
        actorRef.tell(
            (() => { command.run(); counterActor.tell(-1, actorRef) }): Runnable, 
            actorRef)
    }
    ...
    def waitForFinish() = {
        while (counter.get() > 0) {} // now need to sort out this bit
    }
