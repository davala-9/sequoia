...
### file%3A%2F%2F%2Fhome%2Fpigeon%2FDownloads%2Fmsc%2FMSc-Project%2Fsequoia%2Freasoner-kernel%2Fsrc%2Fmain%2Fscala%2Fcom%2Fsequoiareasoner%2Fkernel%2Fcontext%2FContextThreadPool.scala:52: error: `identifier` expected but `]` found
        val count: Int = countfuture.value.get.asInstanceOf[]
                                                            ^
...
class ActorExecutionService extends ForkJoinPool {
    ...
    def waitForFinish(): Future[Boolean] = {
        val countfuture: Future[Any] = Await.result(counterActor ? 0, Duration(10, MINUTES))
        val count: Int = countfuture.value.get.asInstanceOf[]
        println("countget" + count.value.get)
        while (count > 0) {
